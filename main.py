...
def make_coordinates(image, line_parameters):
    ...
    if slope == 0:  # Vermeidung der Division durch Null
        slope = 0.1
    y1 = image.shape[0]
    y2 = int(y1 * (3 / 5))
...
def average_slope_intercept(image, lines):
    ...
    for line in lines:
        ...
        if slope < 0:  # Negative Steigung für linke Linien
            left_fit.append((slope, intercept))
        else:  # Positive Steigung für rechte Linien
...
# Initialisieren Sie den VideoWriter
...
if not cap.isOpened():
    ...
    while True:
        ...
        if not ret:
            ...
            break  # Beendet die Schleife, wenn das Video zu Ende ist oder ein Fehler auftritt
